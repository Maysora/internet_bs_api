...
module InternetBsApi
  class Contact
    ...
    ATTRIBUTES = [:first_name, :last_name, :organization, :email, :phone_number, :fax, :street, :street2, :street3, :city, :country_code, :postal_code, :contact_type]
    ...
    def attributes
      Hash[*(ATTRIBUTES.zip(ATTRIBUTES.map{|attr|self.send attr})).flatten]
    end
    ...
    def to_query(contact_type)
      ...
      if bad_options.length > 0
        ...
        raise InternetBsApi::InvalidContactException.new(exception_string)
      end
      ...
      options << "#{contact_type}Fax=#{fax}" if fax
    ...
    def to_options
      ...
      options_hash["#{contact_type}Fax"] = "#{fax}" if fax
    ...
    def validate
      ...
      bad_options << "#{contact_type}Fax" if fax && validate_phone_number(fax)
